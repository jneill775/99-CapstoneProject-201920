...
class DriveSystem(object):
    ...
    def spin_clock(self, inches, speed):
        """
        Makes the robot go straight at the given speed
        for the given number of inches, using the approximate
        conversion factor of 10.0 inches per second at 100 (full) speed.
        ...
        seconds_per_inch_at_100 = 10.0  # 1 sec = 10 inches at 100 speed
        seconds = abs(float(inches) * seconds_per_inch_at_100 / float(speed))
        self.go_straight_for_seconds(float(seconds), -1*float(speed))
    def spin_anticlock(self, inches, speed):
        """
        Makes the robot go straight at the given speed
        for the given number of inches, using the approximate
        conversion factor of 10.0 inches per second at 100 (full) speed.
    ...
    # -------------------------------------------------------------------------
    def go_forward_until_distance_is_less_than(self, inches,speed,mqtt_sender):
        """
        ...
        while True:
            ...
            if color == 'Black':
                print()
                self.stop()
                mqtt_sender.send_message('show_black', [])
                break
            self.go(speed, speed)
